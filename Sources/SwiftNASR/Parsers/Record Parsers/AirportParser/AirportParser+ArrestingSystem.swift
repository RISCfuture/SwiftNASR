extension AirportParser {
    ...
    func parseArrestingSystemRecord(_ values: [String]) throws {
        ...
        guard let runwayIndex = airport.runways.firstIndex(where: { $0.identification == transformedValues[3] as! String }) else { return }
        let runway = airport.runways[runwayIndex]

        let runwayEndIdentifier = transformedValues[4] as! String
        let runwayEnd: RunwayEndType = if runway.baseEnd.ID == runwayEndIdentifier { .base }
        else if runway.reciprocalEnd?.ID == runwayEndIdentifier { .reciprocal }
        else { throw FixedWidthParserError.invalidValue(runwayEndIdentifier, at: 4) }
