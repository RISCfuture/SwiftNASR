...
final class FrequencyParser: Sendable {
    ...
    private var rx: some RegexComponent {
        Regex {
            ...
            Capture(as: MHzRef) { OneOrMore(.digit) } transform: { .init($0)! }
            Optionally {
                "."
                Capture(as: kHzRef) { OneOrMore(.digit) } transform: { .init($0) }
            }
            Capture(as: useRef) {
            ...
            } transform: { $0.trimmingCharacters(in: .whitespaces).presence }
            Anchor.endOfSubject
        }
    }
