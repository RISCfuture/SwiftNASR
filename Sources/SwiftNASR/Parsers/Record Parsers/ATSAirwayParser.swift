...
class FixedWidthATSAirwayParser: Parser {
  ...
  private func parseATS1(
    ...
    let point = ATSAirway.RoutePoint(
      ...
      distanceToChangeoverPoint: UInt(distCOP),
      trackAngleInbound: trackInbound.isEmpty ? nil : try TrackAnglePair(parsing: trackInbound),
      distanceToNextPoint: Double(distNextPt),
      magneticCourse: magneticCourse,
      magneticCourseOpposite: magneticCourseOpposite,
      minimumEnrouteAltitude: UInt(MEAStr),
      MEADirection: try parseBoundDirection(MEADir),
      MEAOppositeAltitude: UInt(MEAOppStr),
      MEAOppositeDirection: try parseBoundDirection(MEAOppDir),
      maximumAuthorizedAltitude: UInt(maxAlt),
      minimumObstructionClearanceAltitude: UInt(moca),
      hasAirwayGap: gapFlag == "X",
      changeoverPointDistance: UInt(copDist),
      minimumCrossingAltitude: UInt(mca),
      crossingDirection: try parseBoundDirection(mcaDir),
      crossingAltitudeOpposite: UInt(mcaOpp),
  ...
  private func parseATS2(_ line: String, key: String) throws {
    ...
    point = ATSAirway.RoutePoint(
      ...
      position: position,
      minimumReceptionAltitude: UInt(mraStr),
      navaidIdentifier: navaidId.isEmpty ? nil : navaidId,
      trackAngleOutbound: point.trackAngleOutbound,
      distanceToChangeoverPoint: point.distanceToChangeoverPoint,
      trackAngleInbound: point.trackAngleInbound,
      distanceToNextPoint: point.distanceToNextPoint,
      magneticCourse: point.magneticCourse,
      magneticCourseOpposite: point.magneticCourseOpposite,
      minimumEnrouteAltitude: point.minimumEnrouteAltitude,
      MEADirection: point.MEADirection,
      MEAOppositeAltitude: point.MEAOppositeAltitude,
      MEAOppositeDirection: point.MEAOppositeDirection,
      maximumAuthorizedAltitude: point.maximumAuthorizedAltitude,
      minimumObstructionClearanceAltitude: point.minimumObstructionClearanceAltitude,
      hasAirwayGap: point.hasAirwayGap,
      changeoverPointDistance: point.changeoverPointDistance,
      minimumCrossingAltitude: point.minimumCrossingAltitude,
      crossingDirection: point.crossingDirection,
      crossingAltitudeOpposite: point.crossingAltitudeOpposite,
      ...
      magneticVariation: point.magneticVariation,
      ARTCCIdentifier: point.ARTCCIdentifier,
      GNSS_MEA: point.GNSS_MEA,
      GNSS_MEADirection: point.GNSS_MEADirection,
      GNSS_MEAOpposite: point.GNSS_MEAOpposite,
      GNSS_MEAOppositeDirection: point.GNSS_MEAOppositeDirection,
      DME_DME_IRU_MEA: point.DME_DME_IRU_MEA,
      DME_DME_IRU_MEADirection: point.DME_DME_IRU_MEADirection,
      DME_DME_IRU_MEAOpposite: point.DME_DME_IRU_MEAOpposite,
