...
class NavaidParser: FixedWidthParser {
    ...
    static let layoutFormatOrder: [NavaidRecordIdentifier] = [.basicInfo, .remark, .fixes, .holdingPatterns, .fanMarkers, .VORCheckpoint]
    ...
    private func updateNavaid(_ transformedValues: [Any?], process: (inout Navaid) throws -> Void) throws {
        let ID = NavaidKey(values: transformedValues)
...
private func parseClassDesignator(_ code: String) throws -> Navaid.NavaidClass {
...
isEmptyLoop:
    while !scanner.isAtEnd {
        ...
        let sortedCodes = Navaid.NavaidClass.ClassCode.allCases.sorted { $0.rawValue.count > $1.rawValue.count }
...
private func parseSurveyAccuracy(_ code: String) throws -> Navaid.SurveyAccuracy {
    switch code {
...
private func parseTACAN(_ string: String, fieldIndex: Int) throws -> Navaid.TACANChannel {
...
private func parseServiceVolume(_ string: String) throws -> Navaid.ServiceVolume {
    switch string {
        case "H": return .high
        case "L": return .low
...
private func parseHoldingPattern(_ string: String, fieldIndex: Int) throws -> HoldingPatternID {
