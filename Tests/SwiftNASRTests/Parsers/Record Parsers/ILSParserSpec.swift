  ...
  override class func spec() {
    describe("parse") {
      ...
      it("parses ILS facilities") {
        ...
        expect(anb.runwayLengthFt).to(equal(7000))
        expect(anb.runwayWidthFt).to(equal(150))
        ...
        expect(anb.magneticVariationDeg).to(equal(-4))

        // Verify localizer
        expect(anb.localizer).notTo(beNil())
        expect(anb.localizer?.status).to(equal(OperationalStatus.operationalRestricted))
        expect(anb.localizer?.frequencyKHz).to(equal(111500))
        // Position: 33-35-47.260N = 33*3600 + 35*60 + 47.260 = 120947.26 arc-seconds
        expect(anb.localizer?.position?.latitudeArcsec).to(beCloseTo(120947.26, within: 0.01))
        // 085-50-48.960W = -(85*3600 + 50*60 + 48.960) = -309048.96 arc-seconds
        expect(anb.localizer?.position?.longitudeArcsec).to(beCloseTo(-309048.96, within: 0.01))
        expect(anb.localizer?.distanceFromApproachEndFt).to(equal(-8050))
        expect(anb.localizer?.distanceFromCenterlineFt).to(equal(-1))  // negative = left
        expect(anb.localizer?.position?.elevationFtMSL).to(beCloseTo(612.1, within: 0.1))
        expect(anb.localizer?.courseWidthDeg).to(beCloseTo(5.80, within: 0.01))
        expect(anb.localizer?.distanceFromStopEndFt).to(equal(1050))
        ...
        expect(anb.glideSlope?.angleDeg).to(beCloseTo(3.0, within: 0.01))
        expect(anb.glideSlope?.frequencyKHz).to(equal(332900))
        expect(anb.glideSlope?.position?.elevationFtMSL).to(beCloseTo(590.8, within: 0.1))
        ...
        expect(outerMarker.frequencyKHz).to(equal(211))
        ...
        expect(auo.localizer?.frequencyKHz).to(equal(110100))
