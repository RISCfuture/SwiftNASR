...
final class CSVTerminalCommFacilityFoldSpec: AsyncSpec {
  override static func spec() {
    it("folds radar, military, and airspace data into the matching facility") {
      ...
      expect(facility?.radar).toNot(beNil())
      expect(facility?.radar?.equipment).to(haveCount(2))
      expect(facility?.radar?.equipment.map(\.radarType)).to(equal(["ASR", "BCN"]))
      expect(facility?.radar?.equipment.first?.hours).to(equal("24"))

      // Military operations (MIL_OPS.csv) — code decoded to the TXT agency name.
      expect(facility?.militaryOperator).to(equal("U.S. AIR FORCE"))
      expect(facility?.militaryRadioCall).to(equal("TACO"))
      expect(facility?.militaryOperationsHours).to(equal("0600-2200 MON-FRI"))
      expect(facility?.macpHours).to(equal("24"))
      expect(facility?.pmsvHours).to(equal("18"))

      // Class airspace (CLS_ARSP.csv) — Y/blank flags.
      expect(facility?.airspace).toNot(beNil())
      expect(facility?.airspace?.classC).to(beTrue())
      expect(facility?.airspace?.classB).to(beFalse())
      expect(facility?.airspace?.classD).to(beFalse())
      expect(facility?.airspace?.classE).to(beFalse())
      ...
      expect(diagnostics).to(haveCount(2))
    }
  }
}
