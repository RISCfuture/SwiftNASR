...
class NASRDataSpec: AsyncSpec {
    override class func spec() {
        ...
        beforeEach {
            ...
            decodedData = try await JSONDecoder().decode(NASRDataCodable.self, from: encodedData).makeData()
        }
            ...
            describe("state") {
                it("returns the object") {
                    guard let parsedState = await parsedSFO.state,
                          let decodedState = await decodedSFO.state else { fail(); return }
            ...
            describe("countyState") {
                it("returns the object") {
                    guard let parsedState = await parsedSFO.countyState,
                          let decodedState = await decodedSFO.countyState else { fail(); return }
                    expect(parsedState.postOfficeCode).to(equal("CA"))
                    expect(decodedState.postOfficeCode).to(equal("CA"))
                }
            }
            ...
            describe("boundaryARTCCs") {
                it("returns the object") {
                    guard let parsedARTCCs = await parsedSFO.boundaryARTCCs,
                          let decodedARTCCs = await decodedSFO.boundaryARTCCs else { fail(); return }
            ...
            describe("responsibeARTCCs") {
                it("returns the object") {
                    guard let parsedARTCCs = await parsedSFO.responsibleARTCCs,
                          let decodedARTCCs = await decodedSFO.responsibleARTCCs else { fail(); return }
                    expect(parsedARTCCs.count).to(equal(30))
                    expect(decodedARTCCs.count).to(equal(30))
                }
            }
            ...
            describe("tieInFSS") {
                it("returns the object") {
                    guard let parsedFSS = await parsedSFO.tieInFSS,
                          let decodedFSS = await decodedSFO.tieInFSS else { fail(); return }
            ...
            describe("alernateFSS") {
                it("returns the object") {
                    let parsedFSS = await parsedSFO.alternateFSS,
                        decodedFSS = await decodedSFO.alternateFSS
            ...
            describe("NOTAMIssuer") {
                it("returns the object") {
                    let parsedIssuer = await parsedSFO.NOTAMIssuer,
                        decodedIssuer = await decodedSFO.NOTAMIssuer
            ...
            describe("state") {
                it("returns the object") {
                    guard let parsedState = await parsedZOA.state,
                          let decodedState = await decodedZOA.state else { fail(); return }
                    expect(parsedState.postOfficeCode).to(equal("CA"))
                    expect(decodedState.postOfficeCode).to(equal("CA"))
                }
            }
            ...
            describe("nearestFSSWithTeletype") {
                pending("returns the object") {
                    guard let parsedFSS = await parsedOAK.nearestFSSWithTeletype,
                          let decodedFSS = await decodedOAK.nearestFSSWithTeletype else { fail(); return }
                    expect(parsedFSS.ID).to(equal("OAK"))
                    expect(decodedFSS.ID).to(equal("OAK"))
                }
            }
            ...
            describe("state") {
                it("returns the object") {
                    guard let parsedState = await parsedOAK.state,
                          let decodedState = await decodedOAK.state else { fail(); return }
                    expect(parsedState.postOfficeCode).to(equal("CA"))
                    expect(decodedState.postOfficeCode).to(equal("CA"))
                }
            }
                ...
                describe("state") {
                    it("returns the object") {
                        guard let parsedState = await parsedOAK.commFacilities[0].state,
                              let decodedState = await decodedOAK.commFacilities[0].state else { fail(); return }
                        expect(parsedState.postOfficeCode).to(equal("CA"))
                        expect(decodedState.postOfficeCode).to(equal("CA"))
                    }
                }
        ...
        describe("navaid") {
            ...
            describe("state") {
                it("returns the object") {
                    guard let parsedState = await parsedAST.state else { fail(); return }
            ...
            describe("highAltitudeARTCC") {
                it("returns the object") {
                    guard let parsedARTCC = await parsedAST.highAltitudeARTCC else { fail(); return }
            ...
            describe("lowAltitudeARTCC") {
                it("returns the object") {
                    guard let parsedARTCC = await parsedAST.lowAltitudeARTCC else { fail(); return }
                    expect(parsedARTCC.ID).to(equal("ZAN"))
                }
            }
            ...
            describe("controllingFSS") {
                it("returns the object") {
                    guard let parsedFSS = await parsedAST.controllingFSS else { fail(); return }
